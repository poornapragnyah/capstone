...
class SEEDDVPreprocessor:
    ...
    def create_epochs_enhanced(self, raw):
        ...
        epochs, events = self.create_epochs_with_inferred_events(
            raw, 
            onset_time_first_cue_sec=0.0,  # Adjust if needed
            tmin=-0.2, 
            tmax=10.0,
            baseline=(-0.2, 0.0),
    ...
    def process_subject(self, subject_id, subject_files):
        ...
        try:
            ...
            output_file = self.save_preprocessed_data(epochs, subject_id, events=events)
    ...
    def generate_seeddv_events_with_validation(self, sfreq=1000, onset_time_first_cue_sec=0.0, 
    ...
    def create_epochs_with_inferred_events(self, raw, sfreq=None, onset_time_first_cue_sec=0.0,
        ...
        events, timing_info = self.generate_seeddv_events_with_validation(sfreq=sfreq, 
                                    onset_time_first_cue_sec=onset_time_first_cue_sec)
        
        print(f"  Inferred {len(events)} events with timing info: {timing_info}")
