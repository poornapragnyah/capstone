...
class SEEDDVPreprocessor:
    ...
    def create_epochs_enhanced(self, raw, subject_files):
        ...
        print("  Creating enhanced epochs...")
        
        # Load original data to understand structure better
        original_data = []
        ...
        n_total_videos = full_data.shape[0]
        timepoints_per_video = full_data.shape[2]
        
        # Calculate expected duration per video
        expected_duration = timepoints_per_video / self.sfreq
        print(f"    Expected duration per video: {expected_duration:.1f}s")
        
        # Create more epochs by extracting multiple segments per video
        events = []
        
        # Strategy: Extract multiple overlapping epochs per video for better data utilization
        video_samples = int(expected_duration * raw.info['sfreq'])
        epoch_samples = int(10.2 * raw.info['sfreq'])  # 10s + 0.2s baseline
        
        current_sample = 0
        epoch_id = 1
        
        for video_idx in range(n_total_videos):
            video_start = current_sample
            video_end = current_sample + video_samples
            
            # Extract multiple epochs from each video with shorter intervals
            if expected_duration >= 20:  # If video is long enough
                # Extract more epochs per video with overlap
                step_size = int(8 * raw.info['sfreq'])  # 8 seconds step
                n_possible_epochs = int((video_samples - epoch_samples) / step_size) + 1
                n_epochs_per_video = min(8, max(1, n_possible_epochs))  # At least 1, max 8
                
                for epoch_in_video in range(n_epochs_per_video):
                    epoch_start = video_start + epoch_in_video * step_size
                    
                    if epoch_start + epoch_samples <= video_end and epoch_start + epoch_samples <= raw.n_times:
                        events.append([epoch_start + int(0.2 * raw.info['sfreq']), 0, epoch_id])
                        epoch_id += 1
            else:
                # For shorter videos, create overlapping epochs every 5 seconds
                step_size = int(5 * raw.info['sfreq'])  # 5 seconds step
                n_possible_epochs = int((video_samples - epoch_samples) / step_size) + 1
                
                for epoch_in_video in range(max(1, n_possible_epochs)):
                    epoch_start = video_start + epoch_in_video * step_size
            ...
            current_sample = video_end
        
        if len(events) == 0:
            # Fallback: create epochs every 10 seconds
            total_duration = raw.n_times / raw.info['sfreq']
            n_fallback_epochs = int(total_duration / 10)  # Every 10 seconds
            
            for i in range(n_fallback_epochs):
                start_sample = int(i * 10 * raw.info['sfreq'] + 0.2 * raw.info['sfreq'])
                if start_sample + epoch_samples <= raw.n_times:
                    events.append([start_sample, 0, 1])
            
            print(f"    Using fallback: created {len(events)} epochs")
        
        events = np.array(events)
        print(f"    Created {len(events)} epoch events")
        
        # Create epochs with much more lenient rejection criteria
        epoch_duration = 10.0
        baseline = (-0.2, 0.0)
        
        # Much more lenient rejection criteria
        reject_criteria = {
            'eeg': 300e-6,  # Increased from 150µV to 300µV
        }
        
        flat_criteria = {
            'eeg': 0.5e-6,  # Decreased from 1µV to 0.5µV
        }
        
        try:
            epochs = mne.Epochs(raw, events, event_id={str(i): i for i in range(1, epoch_id)}, 
                            tmin=-0.2, tmax=epoch_duration, 
                            baseline=baseline, preload=True,
                            reject=reject_criteria, flat=flat_criteria,
                            reject_by_annotation=False)  # Don't reject by annotation initially
            
            print(f"    Created {len(epochs)} epochs of {epoch_duration}s each")
            print(f"    Rejected {len(events) - len(epochs)} epochs due to artifacts")
            
            # If still too few epochs, try without any rejection
            if len(epochs) < 3:
                print("    Too few epochs with rejection, trying without rejection...")
                epochs = mne.Epochs(raw, events, event_id={str(i): i for i in range(1, min(epoch_id, len(events)+1))}, 
                                tmin=-0.2, tmax=epoch_duration, 
                                baseline=baseline, preload=True,
    ...
    def generate_seeddv_events(self, sfreq=1000, onset_time_first_cue_sec=5.0):
        """
        Generate precise event timings for SEED-DV dataset based on experimental protocol.
        ...
        events : numpy.ndarray
            MNE-compatible events array of shape (280, 3) with columns [sample, 0, event_id]
            Each event marks the onset of a 10-second video concept segment
        """
        import numpy as np
        ...
        concept_block_duration_sec = hint_duration_sec + video_duration_sec  # 13 seconds
        rest_between_blocks_sec = 30.0
        
        # Calculate total expected events
        total_concepts = n_main_blocks * concepts_per_block  # 280
        
        events = []
        event_id = 1
        
        # Current time tracker (in seconds)
        current_time_sec = onset_time_first_cue_sec
        
        for main_block in range(n_main_blocks):
            print(f"  Generating events for main block {main_block + 1}/7...")
            
            for concept in range(concepts_per_block):
                # Time when hint starts
                hint_start_sec = current_time_sec
                
                # Time when video concept starts (after 3-second hint)
                video_start_sec = hint_start_sec + hint_duration_sec
                ...
                # Move to next concept block
                current_time_sec += concept_block_duration_sec
                event_id += 1
            
            # Add rest period between main blocks (except after the last block)
            if main_block < n_main_blocks - 1:  # Not the last block
                current_time_sec += rest_between_blocks_sec
                print(f"    Added {rest_between_blocks_sec}s rest after block {main_block + 1}")
        
        # Convert to numpy array
        events = np.array(events, dtype=int)
        
        # Validation
        assert events.shape[0] == total_concepts, f"Expected {total_concepts} events, got {events.shape[0]}"
        assert events.shape[1] == 3, f"Events array should have 3 columns, got {events.shape[1]}"
        ...
        print(f"  Total experiment duration: {events[-1, 0]/sfreq + video_duration_sec:.2f}s")
